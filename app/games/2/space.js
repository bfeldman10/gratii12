	...
	function loop() {
		...
		if (splashScreen == true) {
			...
			if (p1.dead == false && p1.collides(gratiicoin)) {
				...
				console.log( 'Space is calling coin grab with token: ' + gameToken );
				//parent.arcade.spaceCrabs.coinGrab(gameToken, 1);
				parent.user.arcadeEvents.push({"gameToken":gameToken, "gameEvent":"coinGrab", "gameID":parent.user.gameInProgressID});
				parent.user.changeGratii(1);
				gratii += 1;
				//score += 25;
				gratiicoin.relocate();
			}
